"""Budget management with per-user and per-team limits."""
...
class BudgetManager:
    ...
    async def check_budget(
        ...
        async with self._lock:
            ...
            daily_limit = self._config.user_budgets.get(
                user_id, self._config.default_daily_limit
            )
            monthly_limit = self._config.team_budgets.get(
                team_id, self._config.default_monthly_limit
            )
            ...
            if team_id:
                ...
                team_limit = self._config.team_budgets.get(
                    team_id, self._config.default_monthly_limit
                )
