...
class Proxy:
    """Orchestrates the full 13-stage request pipeline."""
    ...
    async def handle_chat_completion(self, request: Request) -> JSONResponse | StreamingResponse:
        ...
        if self.budget_mgr and self.cfg.budget.enabled:
            ...
            allowed, status = await self.budget_mgr.check_budget(req_ctx.user_id, req_ctx.team_id, estimated_cost)
            if not allowed:
                remaining = status.daily_limit - status.daily_cost
                self._log_audit(req_ctx, req, None, "budget_exceeded", f"remaining: {remaining:.4f}")
                return _error_response(
                    429,
                    f"budget exceeded: daily={status.daily_cost:.2f}/{status.daily_limit:.2f}, "
                    f"monthly={status.monthly_cost:.2f}/{status.monthly_limit:.2f}",
                )
        ...
        if self.plugin_mgr:
            req = self.plugin_mgr.process_request(req)

        # ── Stage 8: Cache Lookup ─────────────────────────────────────────────
        skip_cache = req.stream
        if self.cache and self.cfg.cache.enabled and not skip_cache:
        ...
        try:
            provider = await self.router.route(req, data_classification=data_classification)
        except Exception as e:
        ...
        try:
            ...
            fb_resp, fb_prov = await self._try_fallback(req, provider.name())
            if fb_resp is None or fb_prov is None:
        ...
        if self.plugin_mgr:
            resp = self.plugin_mgr.process_response(req, resp)
        ...
        if self.budget_mgr and self.cfg.budget.enabled:
            await self.budget_mgr.record_usage(
                req_ctx.user_id, req_ctx.team_id,
                req_ctx.cost, req_ctx.tokens_in + req_ctx.tokens_out,
            )
...
def _messages_text(req: ChatCompletionRequest) -> str:
    return " ".join(m.content for m in req.messages if m.content)


def _extract_tool_calls(resp: ChatCompletionResponse) -> List[dict]:
    """Pull tool_calls from all choices in a non-streamed response."""
    calls: List[dict] = []
    for choice in resp.choices:
        ...
        if raw_tc:
            for tc in raw_tc:
                calls.append(tc if isinstance(tc, dict) else tc.model_dump())
    return calls
