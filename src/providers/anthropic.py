...
class AnthropicProvider(BaseProvider):
    ...
    @staticmethod
    def _to_anthropic_body(req: ChatCompletionRequest, prompt_caching: bool = True) -> dict:
        """Convert an OpenAI-style request to the Anthropic Messages API format.

        When ``prompt_caching=True`` (default), system prompts and the last
        user turn are annotated with ``cache_control`` so Anthropic's prompt
        cache can be leveraged, reducing both latency and cost on repeated
        queries with the same system context.
        """
        ...
        if system_parts:
            joined = "\n\n".join(system_parts)
            if prompt_caching:
                # Wrap system in a content-block list with cache_control so
                # Anthropic caches the system prompt across repeated requests.
                body["system"] = [
                    {
                        "type": "text",
                        "text": joined,
        ...
        # Annotate the last user message for cache read (no write overhead).
        if prompt_caching and messages and messages[-1]["role"] == "user":
            last = messages[-1]
            last["content"] = [
                {
                    "type": "text",
